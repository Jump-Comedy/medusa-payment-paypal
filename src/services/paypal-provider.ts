...
class PayPalProviderService extends AbstractPaymentProcessor {
  ...
  constructor({ logger, cartService }: { logger?: Logger; cartService?: any }, options) {
    // @ts-ignore
      ...
      if (this.cartService_) {
        try {
          ...
          const productTitle = cart.items
            ...
            .join(", ")
          if (productTitle) {
            // PayPal soft_descriptor max is 22 characters
            soft_descriptor = productTitle.substring(0, 22)
          }
        } catch (e) {
          this.logger_?.warn?.(
